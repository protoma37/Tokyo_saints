  ...
  for (let index = 0; index < tokenAccounts.value.length; index++) {
    try{
      ...
      if (tokenAmount.amount === "1" && tokenAmount.decimals === 0) {
        ...
        if (metadata.data.data.creators[0].address === COLLECTION_1
          || metadata.data.data.creators[0].address === COLLECTION_2
          || metadata.data.data.creators[0].address === COLLECTION_3) {
