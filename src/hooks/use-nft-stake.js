...
const getPoolData = async (wallet) => {
    ...
    const poolData = {
        ...
        stakeCollection1 : fetchData.stakeCollection1,
        stakeCollection2 : fetchData.stakeCollection2,
        stakeCollection3 : fetchData.stakeCollection3,
        stakeCollection4 : fetchData.stakeCollection4,
        stakeCollection5 : fetchData.stakeCollection5
    }
    return poolData;
}
...
const stake = async (nfts , wallet) => {
    ...
    for(let nft of nfts) {
        ...
        const typeObject = nft.attributes.filter(item => item.trait_type === "Vigilante Group");
        const type = typeObject[0].value;
        ...
        transaction.push(
            program.instruction.stake(type.toLowerCase(), {
                accounts: {
...
const unstake = async (nfts, wallet) => {
    try{
        ...
        for(let nft of nfts){
            ...
            transaction.push(
                program.instruction.unstake(type.toLowerCase(), {
                    accounts:{
                        owner : wallet.publicKey,
                        pool : pool,
...
async function claim(wallet) {
    ...
    if (result.result) {
        toast.success(`Successfully Claimed ${(amount / LAMPORTS_PER_SOL).toFixed(3)} $AERO`);
    }
}
...
const useNftStake = () => {
    ...
    useEffect(() => {
        (async () => {
            ...
            setDailyRewardAmount((newDailyRewardAmount / LAMPORTS_PER_SOL).toFixed(3));
